...
namespace taco
{
  ...
  public:
    ...
    private:
      ...
      struct Context
      {
        Context(int order, int bufferCapacity, void *iterCtx) : coordBuffer(new T[order * bufferCapacity]),
      ...
      const_iterator(const TensorBase *tensor, bool isEnd = false) : tensor(tensor),
  ...
  void TensorBase::insert(const std::initializer_list<int> &coordinate, CType value)
  {
    ...
    taco_uassert(getComponentType() == type<CType>()) << "Cannot insert a value of type '" << type<CType>() << "' " << "into a tensor with component type " << getComponentType();
  ...
  {
    taco_uassert(coordinate.size() == (size_t)getOrder()) << "Wrong number of indices";
    taco_uassert(getComponentType() == type<CType>()) << "Cannot get a value of type '" << type<CType>() << "' " << "from a tensor with component type " << getComponentType();
    syncValues();
