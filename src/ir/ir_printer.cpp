...
namespace taco
{
  namespace ir
  {
    ...
    // 该模板函数 `acceptJoin` 用于遍历一个节点向量 `nodes`，并将其内容打印到输出流 `stream` 中。
    // - `printer` 是一个指向 `IRPrinter` 的指针，用于访问打印逻辑。
    // - `stream` 是输出流，用于接收打印的内容。
    // - `nodes` 是一个包含节点的向量，每个节点都需要调用其 `accept` 方法进行打印。
    // - `sep` 是分隔符字符串，用于在节点之间插入分隔符。

    // 函数逻辑：
    // 1. 如果 `nodes` 非空，先打印第一个节点（无分隔符）。
    // 2. 从第二个节点开始，依次打印分隔符 `sep` 和节点内容。
    // 该函数常用于打印类似逗号分隔的列表（如函数参数、数组元素等）。
    template <class T>
    ...
    void IRPrinter::visit(const Literal *op)
    {
      ...
      case Datatype::Float32:
        stream << ((op->getValue<float>() != 0.0)
                       ? util::toString(op->getValue<float>())
                       : "0.0");
        break;
      case Datatype::Float64:
        stream << ((op->getValue<double>() != 0.0)
                       ? util::toString(op->getValue<double>())
                       : "0.0");
        break;
    ...
    void IRPrinter::visit(const Var *op)
    {
      ...
      {
        stream << varNames.get(op); // Scoped map 会重写变量名？防止冲突？
      }
      else
      {
        stream << op->name;
    ...
    void IRPrinter::visit(const Function *op) // 默认所有的函数无返回值，且所有参数的类型均为 Tensor ? 
    {
    ...
    void IRPrinter::resetNameCounters()
    {
      ...
      vector<string> keywords =
          {"auto",
           ...
           "complex",
           "imaginary"};
      varNameGenerator = util::NameGenerator(keywords); /// 有些保留字不能用！
    }
