...
namespace taco
{
  namespace ir
  {
    ...
    Expr IRRewriter::rewrite(Expr e) // top
    {
      if (e.defined())
      {
        e.accept(this); // write to expr, the result of visit methods is stored in expr!
        e = expr;       // Expr is a smart ptr indeed!
      }
      else
      {
        e = Expr();
    ...
    Stmt IRRewriter::rewrite(Stmt s) // top
    {
      if (s.defined())
      {
        s.accept(this);
    ...
    void IRRewriter::visit(const Sizeof *op) //  the ptr of IRNOde can used to initialize expr!
    {
      expr = op;
    }
    ...
    void IRRewriter::visit(const Yield *op) // 语法是什么?
    {
