...
namespace taco
{
  namespace ir
  {
    ...
    {
      using IRRewriter::visit; /// 只是重写 visit 方法，不需要重写 rewriter !
      ...
      void visit(const Add *op) // Essentially, precompute all possible literal !
      {
        Expr a = rewrite(op->a);
        Expr b = rewrite(op->b);
        ...
        if (isa<Literal>(a) && isa<Literal>(b))
        {
          ...
          if (isScalar(typea) && isScalar(typeb) && (resulttype.isInt() || resulttype.isUInt()))
          {
            ...
            if (typea != resulttype)
            {
              litaval = TypedComponentVal(resulttype, (int)litaval.getAsIndex()); // ??? int 对吗？
            }
            if (typeb != resulttype)
            {
              litbval = TypedComponentVal(resulttype, (int)litbval.getAsIndex()); // ??? int 对吗？
            }
    ...
    ir::Stmt simplify(const ir::Stmt &stmt)
    {
      ...
      {
        std::set<Expr> loopDependentVars; /// 储存两类值，一个是循环变量，一个是在循环内部会被修改的值。
        std::map<Expr, int> defLevel;     /// 给定Expr返回其在哪个 loop level 被定义的
        int loopLevel = 0;
      ...
      struct RemoveRedundantStmts : public IRRewriter
      {
        ...
        void visit(const VarDecl *decl)
        {
          if (isa<ir::Call>(decl->rhs))
          { // don't remove function calls that might have side effects
            stmt = decl;
            return;
          }
