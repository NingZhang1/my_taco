...
namespace taco
{
  ...
  std::vector<ir::Expr> IndexVarRelNode::computeRelativeBound(std::set<IndexVar> definedVars,                           //
  ...
  ir::Expr IndexVarRelNode::recoverVariable(IndexVar indexVar,                                           //
                                            std::map<IndexVar, ir::Expr> variableNames,                  //
                                            Iterators iterators,                                         //
  ...
  ir::Stmt IndexVarRelNode::recoverChild(IndexVar indexVar, std::map<IndexVar, ir::Expr> variableNames, //
                                         bool emitVarDecl,                                              //
                                         Iterators iterators, ProvenanceGraph provGraph) const
  {
    taco_ierror;
  ...
  // 返回一系列 expression 计算 relative bound !! , 返回两个数值！
  std::vector<ir::Expr> SplitRelNode::computeRelativeBound(std::set<IndexVar> definedVars,                           //
                                                           std::map<IndexVar, std::vector<ir::Expr>> computedBounds, //
                                                           std::map<IndexVar, ir::Expr> variableExprs,               //
                                                           Iterators iterators, ProvenanceGraph provGraph) const
  {
    ...
    if (!outerVarDefined && !innerVarDefined) // 都没有定义！
    {
      return parentBound;
    }
  ...
  std::vector<ir::Expr> SplitRelNode::deriveIterBounds(taco::IndexVar indexVar,                                     //
                                                       std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds,  //
                                                       std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, // not used!
                                                       std::map<taco::IndexVar, taco::ir::Expr> variableNames,      //
                                                       Iterators iterators, ProvenanceGraph provGraph) const
  {
  ...
  ir::Expr SplitRelNode::recoverVariable(taco::IndexVar indexVar,                                                                                                 //
                                         std::map<taco::IndexVar, taco::ir::Expr> variableNames,                                                                  //
                                         Iterators iterators,                                                                                                     //
                                         std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, //
                                         ProvenanceGraph provGraph) const
  {
  ...
  std::vector<ir::Expr> DivideRelNode::computeRelativeBound(std::set<IndexVar> definedVars,                           //
                                                            std::map<IndexVar, std::vector<ir::Expr>> computedBounds, //
                                                            std::map<IndexVar, ir::Expr> variableExprs,               //
                                                            Iterators iterators, ProvenanceGraph provGraph) const     // provGraph 记录了每个 IndexVar 的性质？
  {
    taco_iassert(computedBounds.count(getParentVar()) == 1);
    std::vector<ir::Expr> parentBound = computedBounds.at(getParentVar());
    bool outerVarDefined = definedVars.count(getOuterVar());
  ...
  std::vector<ir::Expr> PosRelNode::computeRelativeBound(std::set<IndexVar> definedVars,                           //
                                                         std::map<IndexVar, std::vector<ir::Expr>> computedBounds, //
                                                         std::map<IndexVar, ir::Expr> variableExprs,               //
                                                         Iterators iterators, ProvenanceGraph provGraph) const
  {
  ...
  ProvenanceGraph::ProvenanceGraph(IndexStmt concreteStmt)
  {
    ...
    match(concreteStmt,
          std::function<void(const ForallNode *)>([&](const ForallNode *op)
                                                  { nodes.insert(op->indexVar); })); // 抽出所有 forall 的指标？

    // Get SuchThat node with relations, why 必须是 such that !
    ...
    SuchThat suchThat = to<SuchThat>(concreteStmt);          // such that node 包含了 一个 indexstmt 以及记录指标的关系 ?
    vector<IndexVarRel> relations = suchThat.getPredicate(); // predicate : 谓语 the part of a sentence or clause containing a verb and stating something about the subjec
  ...
  std::vector<IndexVar> ProvenanceGraph::getFullyDerivedDescendants(IndexVar indexVar) const // children of children ?
  {
  ...
  std::vector<IndexVar> ProvenanceGraph::getUnderivedAncestors(IndexVar indexVar) const // 似乎只是抽取出 father of father ?
  {
  ...
  bool ProvenanceGraph::getPosIteratorDescendant(IndexVar indexVar, IndexVar *irregularChild) const
  {
    ...
    if (childRelMap.at(indexVar).getRelType() == FUSE && isPosVariable(getChildren(indexVar)[0]))
    { // can't gain pos by fusing with pos variable
      return false;
    }
    ...
    for (IndexVar child : getChildren(indexVar))
    {
      if (!util::contains(childRelMap.at(indexVar).getNode()->getIrregulars(), child))
      { // is irregularity not maintained through relationship
  ...
  bool ProvenanceGraph::getPosIteratorFullyDerivedDescendant(IndexVar indexVar, IndexVar *irregularChild) const
  {
    ...
    if (childRelMap.at(indexVar).getRelType() == FUSE && isPosVariable(indexVar))
    { // can't iterate pos through fuse
      return false;
    }
                                                                 ...
                                                                 std::set<taco::IndexVar> previouslyDefined) const
  {
    ...
    for (const IndexVar &parent : getParents(indexVar))
    {
      if (parentRelMap.at(indexVar).getRelType() == FUSE)
      {
        ...
        if (getIrregularDescendant(indexVar, &irregularDescendant) && isPosVariable(irregularDescendant) && isCoordVariable(parent))
        { // Fused Pos case needs to be tracked with special while loop
