...
namespace taco
{
  ...
  AttrQuery::Attr::Attr(
      std::tuple<std::string, Aggregation, std::vector<IndexVar>> attr) : label(std::get<0>(attr)), aggr(std::get<1>(attr)),
  ...
  ModeFormatImpl::ModeFormatImpl(const std::string name, bool isFull,
                                 ...
                                 bool isYieldPosPure) : name(name), isFull(isFull), isOrdered(isOrdered), isUnique(isUnique),
