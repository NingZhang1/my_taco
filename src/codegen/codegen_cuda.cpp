  ...
  namespace ir
  {
    ...
    protected:
      ...
      virtual void visit(const Var *op)
      {
        ...
        else if (scopeMap.count(op) == 1 && inDeviceFunction && currentParameterSet.count(op) == 0 && (threadIDVars.empty() || op != threadIDVars.back().second) && (blockIDVars.empty() || op != blockIDVars.back().second) && (warpIDVars.empty() || op != warpIDVars.back().second) && !variablesDeclaredInKernel.count(op))
        {
