...
namespace taco
{
                         ...
                         Format format, Literal fill)
  {
    ...
    taco_uassert((size_t)format.getOrder() == dimensions.size()) << "The number of format mode types (" << format.getOrder() << ") " << "must match the tensor order (" << dimensions.size() << ").";
        ...
        : AccessNode(tensor.getTensorVar()), tensor(tensor)
    {
      ...
      for (size_t i = 0; i < indices.size(); i++)
      {
        ...
        IndexVarInterface::match(var, [&](std::shared_ptr<IndexVar> ivar)
                                 { ivars[i] = *ivar; }, [&](std::shared_ptr<WindowedIndexVar> wvar)
                                 {
        ...
        this->windowedModes[i].stride = wvar->getStride(); }, [&](std::shared_ptr<IndexSetVar> svar)
                                 {
